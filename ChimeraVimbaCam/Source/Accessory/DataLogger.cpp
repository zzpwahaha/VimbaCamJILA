...
DataLogger::DataLogger(std::string systemLocation, cameraMainWindow* parent) : IChimeraSystem(parent){
...
*/
void DataLogger::initializeDataFiles( std::string filePath, bool checkForCalibrationFiles ){
...
void DataLogger::normalCloseFile(){
	...
	try	{
		// log the close time.
		try {
			file.openGroup("/Miscellaneous");
		}
		catch (H5::Exception& e) {
			// group does not exist, create it
			file.createGroup("/Miscellaneous");
		}
